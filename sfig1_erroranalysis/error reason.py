...
import sys
sys.path.append("F:/work/work5_reductionability/NetworkReduction")
...
if __name__ == '__main__':
    ...
    for i,beta in enumerate(p):
        ...
        for j in np.arange(simulation):
            [S,I,R]= cg.SIR_TS_Nodes(G, beta,mu, infecteds[0])
            time_series = []
            for each in R:
                time_series.extend(each)
            if 'F' in time_series:
                 count = count + 1
    ...
    y4 = np.zeros(len(p))
    for i,beta in enumerate(p):
        s[i] = 4*beta**9 - 19*beta**8 + 32*beta**7 - 19*beta**6 - 3*beta**5 + 4*beta**4 + 2*beta**3
        m[i] = 2*beta**2-beta**3#beta*(1-np.power((1-beta),2))
        y4[i] = -6*beta**6 + 24*beta**5 - 33*beta**4 + 16*beta**3
        
    x_index = np.min(np.where(y4>0.995)[0])/100
    x_equal = np.where((m - s)<0.005)[0][6]/100
    ...
    ax[2].plot(p[x],s[x],'s',label = r'$p(A \rightarrow F|G)$',markersize=marker_size,mec='white',color=colors(1))#2\beta^3 +  4\beta^4 - 3\beta^5 - 19\beta^6  + 32\beta^7 - 19\beta^8+4\beta^9 $
    ax[2].plot(p[x],result[x],'-',label = r'$p(A \rightarrow F|G)$',lw=lw,mec=mec_color,color=mec_color)
    ax[2].plot(p[x],m[x],'^',label = r'$p(A \rightarrow F|G_{R=4})$',markersize=marker_size,mec='white',color=colors(2))#2\beta^2-\beta^3
    ax[2].plot(p[x],y4[x],'o-',label = r'$\Lambda^4_{1 \rightarrow 3}(\beta)$',markersize=marker_size,mec=mec_color,color=colors(0))# $-6\beta^6 + 24\beta^5 - 33\beta^4 + 16\beta^3$
    #ax[2].vlines(pc,0,0.75,color=mec_color,linestyles='dashed')
